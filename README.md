# chirp

This project is a **Social Application** for the purpose of sharing thoughts and idea. Built with Flutter.
It demonstrates my ability to **design real world mobile app** with clean layouts, reusable components and responsive design

## Features

* **Responsive Layouts**: responsive design ensuring seamless adaptability across varying screen dimensions.
  
* **Post**: Text, pictures, video, audio and other files like PDF usually with a size limits.
  
* **Search**: Keyword search for profiles and post.
  
* **Social Interactions**: Engagement services (follow, like, comment and share).
  
* **Collections**: Enable users to save, manage, and easily access ideas based on the post categories.
  
* **Conversation Bubbles**: engineered with a persistent timestamp architecture, ensuring temporal markers remain consistently anchored at the bottom-right for enhanced readability and interface precision.
  
* **Dark Mode Support**: comprehensive dark mode integration, delivering an adaptive interface that optimizes visual comfort, preserves aesthetic consistency and enhances readability across diverse lighting environment.
  
* **Custom Reusable Widgets**: scalable and maintainable codebase.
  
* **Clean Project Structure**: easy to extend into a full app.

## Tech Stack

* **Flutter**(UI framework)
* **Dart** (language)
* **Firebase**(backend)

## Screenshots

### Home

| Light Mode | Dark Mode |
|:-----------:|:-----------:|
| <img src="https://github.com/teliej/chirp/blob/main/assets/Screenshots/feed_light.jpg" width="300"> | <img src="https://github.com/teliej/chirp/blob/main/assets/Screenshots/feed_dark.jpg" width="300"> |

| Profile | Video Demo |
|:-----------:|:-----------:|
| <img src="https://github.com/teliej/chirp/blob/main/assets/Screenshots/profile_dark.jpg" width="300"> | <video src="https://github.com/teliej/chirp/blob/72dde0920aee74a3815e5ba5b1ec35f4436d2951/chirp.mp4" width="300"> |
